-- Estabiliza a RPC dentro do statement_timeout do PostgREST.

CREATE INDEX IF NOT EXISTS idx_catalogo_tipo_objeto_id
  ON public.catalogo (tipo_objeto, id);

CREATE INDEX IF NOT EXISTS idx_catalogo_descricao_prefix_lower_pattern
  ON public.catalogo ((left(lower(descricao), 160)) text_pattern_ops);
...
BEGIN
  ...
  IF normalized_query = '' THEN
    ...
    WHERE
      (normalized_category = 'all' AND c.tipo_objeto IS NOT NULL)
  ...
  SELECT
    ...
    c.unidade_medida, 0.8::REAL AS rank
  FROM public.catalogo c
  WHERE left(lower(c.descricao), 160) LIKE normalized_query || '%'
    AND (
      normalized_category = 'all'
      OR (normalized_category = 'material' AND c.tipo_objeto = 'MATERIAL')
      OR (normalized_category = 'servico' AND c.tipo_objeto = 'SERVIÇO')
