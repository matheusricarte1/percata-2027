-- Separa ramos full-text por categoria para evitar plano generico.
...
BEGIN
  ...
  ELSE
    RETURN QUERY SELECT c.id, c.codigo_efisco, c.descricao, c.tipo, c.categoria, c.grupo, c.classe, c.tipo_objeto, c.codigo_grupo, c.nome_grupo, c.codigo_classe, c.nome_classe, c.codigo_material_servico, c.nome_material_servico, c.codigo_natureza_preferencial, c.gnd_preferencial, c.natureza_count, c.unidade_medida, ts_rank_cd(c.search_vector, terms)::REAL FROM public.catalogo c WHERE c.search_vector @@ terms ORDER BY ts_rank_cd(c.search_vector, terms) DESC, c.id ASC LIMIT limit_val OFFSET offset_val;
  END IF;
END;
$$ LANGUAGE plpgsql STABLE;

ANALYZE public.catalogo;
