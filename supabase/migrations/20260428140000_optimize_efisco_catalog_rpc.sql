-- Otimiza busca do catalogo e-Fisco para PostgREST.
-- A busca por codigo nao deve acionar ranking textual sobre toda a tabela.

CREATE INDEX IF NOT EXISTS idx_catalogo_codigo_efisco_lower
  ON public.catalogo (lower(codigo_efisco));
...
BEGIN
  ...
  IF is_code_query THEN
    ...
    WITH scored AS (
      SELECT
        ...
        CASE WHEN lower(coalesce(c.codigo_efisco, '')) LIKE '%' || normalized_query || '%' THEN 1 ELSE 0 END AS contains_code
      FROM public.catalogo c
      WHERE
        (
          normalized_category = 'all'
        ...
        AND lower(coalesce(c.codigo_efisco, '')) LIKE '%' || normalized_query || '%'
    )
    SELECT
      ...
      s.unidade_medida,
      ((s.exact_code * 1.6) + (s.prefix_code * 0.8) + (s.contains_code * 0.25))::REAL AS rank
    FROM scored s
    ORDER BY s.exact_code DESC, s.prefix_code DESC, s.codigo_efisco ASC
    LIMIT limit_val
    OFFSET offset_val;
  ...
  SELECT
    ...
    s.codigo_natureza_preferencial, s.gnd_preferencial, s.natureza_count,
    s.unidade_medida,
    ((s.phrase_desc * 0.35) + (s.taxonomy_match * 0.2) + (s.ts_rank * 0.55))::REAL AS rank
  FROM scored s
