-- Evita timeout em listagem inicial e busca textual do catalogo.
...
BEGIN
  ...
  WITH scored AS (
    ...
    WHERE
      ...
      AND to_tsvector('portuguese', coalesce(c.descricao, '')) @@ query_terms
  )
  SELECT
    ...
    s.unidade_medida, s.ts_rank::REAL AS rank
  FROM scored s
  ORDER BY s.ts_rank DESC, s.id ASC
  LIMIT limit_val
  OFFSET offset_val;
END;
