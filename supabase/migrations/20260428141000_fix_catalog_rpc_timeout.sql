-- Remove varredura por substring na busca de codigo para evitar timeout no PostgREST.

CREATE INDEX IF NOT EXISTS idx_catalogo_codigo_efisco_lower_pattern
  ON public.catalogo (lower(codigo_efisco) text_pattern_ops);
...
BEGIN
  ...
  IF normalized_query = '' THEN
    ...
    ORDER BY
      CASE WHEN c.codigo_efisco ~ '^[0-9]+-[0-9]+$' THEN 0 ELSE 1 END,
      c.id ASC
    LIMIT limit_val
    OFFSET offset_val;
  ...
  IF is_code_query THEN
    ...
    SELECT
      ...
      c.unidade_medida,
      CASE
        WHEN lower(coalesce(c.codigo_efisco, '')) = normalized_query THEN 1.6
        ELSE 0.8
      END::REAL AS rank
    FROM public.catalogo c
    WHERE
      ...
      AND (
        lower(coalesce(c.codigo_efisco, '')) = normalized_query
        OR lower(coalesce(c.codigo_efisco, '')) LIKE normalized_query || '%'
      )
    ORDER BY rank DESC, c.codigo_efisco ASC
    LIMIT limit_val
    OFFSET offset_val;
  ...
  SELECT
    ...
    s.unidade_medida,
    ((s.phrase_desc * 0.35) + (s.ts_rank * 0.55))::REAL AS rank
  FROM scored s
