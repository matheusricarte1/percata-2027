-- Ajusta busca por codigo e ranqueamento de sinonimos para reduzir falsos positivos.
...
VALUES
  ...
  ('notebook', 'notebook|microcomputador|computador portatil|laptop'),
  ('laptop', 'notebook|microcomputador|computador portatil|laptop'),
  ('computador portatil', 'notebook|microcomputador|computador portatil|laptop'),
  ('ar condicionado', 'ar condicionado|condicionador de ar|aparelho de ar condicionado|refrigeracao|climatizacao|climatizador'),
  ('condicionador', 'ar condicionado|condicionador de ar|aparelho de ar condicionado|refrigeracao|climatizacao|climatizador'),
  ('climatizador', 'ar condicionado|condicionador de ar|refrigeracao|climatizacao'),
  ('dedetizacao', 'dedetizacao|desinfestacao|desinsetizacao|controle de pragas|insetos|ratos|cupins'),
  ('dedetização', 'dedetizacao|desinfestacao|desinsetizacao|controle de pragas|insetos|ratos|cupins'),
  ('desinsetizacao', 'dedetizacao|desinfestacao|desinsetizacao|controle de pragas|insetos'),
  ('controle pragas', 'dedetizacao|desinfestacao|desinsetizacao|controle de pragas|insetos|ratos|cupins')
ON CONFLICT (term_norm)
DO UPDATE SET expansion_norm = EXCLUDED.expansion_norm;

CREATE INDEX IF NOT EXISTS idx_catalogo_codigo_efisco_key
  ON public.catalogo ((regexp_replace(lower(coalesce(codigo_efisco, '')), '[^a-z0-9]', '', 'g')));
...
AS $$
  ...
  phrase_parts AS (
    SELECT trim(regexp_split_to_table(expansion_norm, '\s*\|\s*')) AS part
...
BEGIN
  ...
  ELSIF expansion_text ~
    '(material|equipamento|consumo|peca|mobiliario|medicamento|computador|notebook|microcomputador|condicionador de ar|aparelho de ar condicionado)'
  THEN
  ...
  IF normalized_query <> '' AND is_code_query THEN
    ...
    WHERE regexp_replace(lower(coalesce(c.codigo_efisco, '')), '[^a-z0-9]', '', 'g') = code_query
      AND (
    ...
    FROM public.catalogo c
    WHERE regexp_replace(lower(coalesce(c.codigo_efisco, '')), '[^a-z0-9]', '', 'g') LIKE code_query || '%'
      AND (
        normalized_category = 'all'
        OR (normalized_category = 'material' AND c.tipo_objeto = 'MATERIAL')
        OR (normalized_category = 'servico' AND c.tipo_objeto = 'SERVIÇO')
